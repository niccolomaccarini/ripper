...
from wittgenstein4.base_functions import truncstr
...
class BinTransformer:
    ...
    def transform(self, df, ignore_feats=[]):
        """Return df with any continuous features binned, and the bin_transformer or None depending on whether binning occurs."""

        if self.n_discretize_bins is None:
            return df
    ...
    def fit(self, df, output=False, ignore_feats=[]):
        ...
        if feats_to_fit:
            ...
            if self.verbosity == 2:
                print(f"fitting bins for {len(feats_to_fit)} features: {feats_to_fit}")
            if self.verbosity >= 2:
                print()
